...
class DanceSelect(tk.Frame):
    def __init__(self, parent, controller):
        ...
        defaultButton = tk.Button(self.middle_frame, image=self.defaultDance, font=("Terminal", 16), bg="#A020F0", command=lambda: self.controller.show_dance("Default"))
        ...
        gangnamButton = tk.Button(self.middle_frame, image=self.gangnamDance, font=("Terminal", 16), bg="#A020F0", command=lambda: self.controller.show_dance("Gangnam"))
        gangnamButton.pack(side=tk.LEFT, pady=(0,40), expand=True)
        griddyButton = tk.Button(self.middle_frame, image=self.griddyDance, font=("Terminal", 16), bg="#A020F0", command=lambda: self.controller.show_dance("Griddy"))
        griddyButton.pack(side=tk.LEFT, pady=(0,40), expand=True)
        orangeButton = tk.Button(self.middle_frame, image=self.orangeDance, font=("Terminal", 16), bg="#A020F0", command=lambda: self.controller.show_dance("Orange"))
        orangeButton.pack(side=tk.LEFT, pady=(0,40), expand=True)
        takeTheLButton = tk.Button(self.middle_frame, image=self.takeTheL, font=("Terminal", 16), bg="#A020F0", command=lambda: self.controller.show_dance("TakeTheL"))
